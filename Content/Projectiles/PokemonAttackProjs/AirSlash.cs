...
namespace Pokemod.Content.Projectiles.PokemonAttackProjs
{
	public class AirSlash : PokemonAttack
	{
		...
		public override void AttackOutTimer(Projectile pokemon, float distanceFromTarget, Vector2 targetCenter){
			...
			if(pokemon.owner == Main.myPlayer){
				if(pokemonOwner.currentStatus == (int)PokemonPetProjectile.ProjStatus.Attack && pokemonOwner.timer%10 == 0){
					for(int i = 0; i < pokemonOwner.nAttackProjs; i++){
						if(pokemonOwner.attackProjs[i] == null){
							pokemonOwner.attackProjs[i] = Main.projectile[Projectile.NewProjectile(Projectile.InheritSource(pokemon), pokemon.Center, 22f*Vector2.Normalize(targetCenter-pokemon.Center), ModContent.ProjectileType<AirSlash>(), pokemonOwner.GetPokemonAttackDamage(GetType().Name), 2f, pokemon.owner)];
