...
{
    public class AquaTail : PokemonAttack
	{
			...
			if(pokemon.owner == Main.myPlayer){
				for(int i = 0; i < pokemonOwner.nAttackProjs; i++){
					if(pokemonOwner.attackProjs[i] == null){
						pokemonOwner.attackProjs[i] = Main.projectile[Projectile.NewProjectile(Projectile.InheritSource(pokemon), pokemon.Center, Vector2.Zero, ModContent.ProjectileType<AquaTail>(), pokemonOwner.GetPokemonAttackDamage(GetType().Name), 0f, pokemon.owner)];
        ...
        {
            var pokemonOwner = (PokemonPetProjectile)pokemon.ModProjectile;
            AquaTail proj = (AquaTail)pokemonOwner.attackProjs[i].ModProjectile;
