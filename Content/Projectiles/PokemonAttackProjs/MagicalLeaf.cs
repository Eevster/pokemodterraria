...
namespace Pokemod.Content.Projectiles.PokemonAttackProjs
{
	public class MagicalLeaf : PokemonAttack
	{
		...
		public override void AttackOutTimer(Projectile pokemon, float distanceFromTarget, Vector2 targetCenter){
			...
			if(pokemon.owner == Main.myPlayer){
				if(pokemonOwner.currentStatus == (int)PokemonPetProjectile.ProjStatus.Attack && pokemonOwner.timer%20==0){
					for(int i = 0; i < pokemonOwner.nAttackProjs; i++){
						if(pokemonOwner.attackProjs[i] == null){
							SoundEngine.PlaySound(SoundID.Item4, Projectile.position);
							pokemonOwner.attackProjs[i] = Main.projectile[Projectile.NewProjectile(Projectile.InheritSource(pokemon), pokemon.Center, new Vector2(Main.rand.NextFloat(-10,10),-10), ModContent.ProjectileType<MagicalLeaf>(), (int)(0.5f*pokemonOwner.GetPokemonAttackDamage(GetType().Name)), 2f, pokemon.owner)];
							break;
						}
					} 
				}
