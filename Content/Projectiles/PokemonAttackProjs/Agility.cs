...
{
	public class Agility : PokemonAttack
	{
        public override bool CanExistIfNotActualMove => false;
        private const float amplitude = 48;
        private float xPos = 0;
        private float xVel = 0;
        private const float maxVel = 50;
        private const float xAcc = 2f;
			...
			if(pokemon.owner == Main.myPlayer){
				for(int i = 0; i < pokemonOwner.nAttackProjs; i++){
					if(pokemonOwner.attackProjs[i] == null){
						pokemonOwner.attackProjs[i] = Main.projectile[Projectile.NewProjectile(Projectile.InheritSource(pokemon), pokemon.Center, Vector2.Zero, ModContent.ProjectileType<Agility>(), 0, 0f, pokemon.owner,  targetCenter.X, targetCenter.Y, Projectile.scale)];
						SoundEngine.PlaySound(SoundID.Item115, pokemon.position);
