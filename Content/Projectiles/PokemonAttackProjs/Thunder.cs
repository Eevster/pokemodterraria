...
{
	public class Thunder : PokemonAttack
	{
        bool shooted = false;
		...
		public override void AttackOutTimer(Projectile pokemon, float distanceFromTarget, Vector2 targetCenter){
			...
			if(pokemon.owner == Main.myPlayer){
				if(pokemonOwner.currentStatus == (int)PokemonPetProjectile.ProjStatus.Attack && pokemonOwner.timer > (pokemonOwner.attackDuration/2)){
					if(pokemonOwner.timer%5 == 4){
						for(int i = 0; i < pokemonOwner.nAttackProjs; i++){
							if(pokemonOwner.attackProjs[i] == null){
								pokemonOwner.attackProjs[i] = Main.projectile[Projectile.NewProjectile(Projectile.InheritSource(pokemon), targetCenter + new Vector2(i%3==0?0:(i%3==1?-32:32),-400), Vector2.Zero, ModContent.ProjectileType<Thunder>(), pokemonOwner.GetPokemonAttackDamage(GetType().Name), 2f, pokemon.owner)];
								SoundEngine.PlaySound(SoundID.Item8, pokemon.position);
								break;
							}
						}
					}
        ...
        {
			if(Main.rand.NextBool(10)) target.AddBuff(ModContent.BuffType<ParalyzedDebuff>(), (target.boss?2:3)*60);
            base.OnHitNPC(target, hit, damageDone);
        }

        public override void OnHitPlayer(Player target, Player.HurtInfo info)
        {
			if(Main.rand.NextBool(10)) target.AddBuff(ModContent.BuffType<ParalyzedDebuff>(), 2*60);
