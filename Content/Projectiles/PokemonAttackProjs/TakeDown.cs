...
{
	public class TakeDown : PokemonAttack
	{
			...
			if(pokemon.owner == Main.myPlayer){
				for(int i = 0; i < pokemonOwner.nAttackProjs; i++){
					if(pokemonOwner.attackProjs[i] == null){
						pokemonOwner.attackProjs[i] = Main.projectile[Projectile.NewProjectile(Projectile.InheritSource(pokemon), pokemon.Center, Vector2.Zero, ModContent.ProjectileType<TakeDown>(), pokemonOwner.GetPokemonAttackDamage(GetType().Name) * 2, 12f, pokemon.owner)];
        ...
        public override void AfterHitTarget(Entity target, int damageDone)
        {
			if (!didHit)
			{
				...
				for (int i = 0; i < 20; i++)
				{
					int dustIndex = Dust.NewDust(Projectile.Center - new Vector2(2, 2), 4, 4, DustID.DirtSpray, 0, 0, 50, default(Color), 1f);
