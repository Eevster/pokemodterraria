...
{
	public class FlameCharge : PokemonAttack
	{
			...
			if(pokemon.owner == Main.myPlayer){
				for(int i = 0; i < pokemonOwner.nAttackProjs; i++){
					if(pokemonOwner.attackProjs[i] == null){
						pokemonOwner.attackProjs[i] = Main.projectile[Projectile.NewProjectile(Projectile.InheritSource(pokemon), pokemon.Center, Vector2.Zero, ModContent.ProjectileType<FlameCharge>(), pokemonOwner.GetPokemonAttackDamage(GetType().Name) * 2, 0f, pokemon.owner)];
						pokemon.velocity = 36*Vector2.Normalize(targetCenter-pokemon.Center);
						SoundEngine.PlaySound(SoundID.Item60, pokemon.position);
						pokemonOwner.timer = pokemonOwner.attackDuration;
						pokemonOwner.ApplyStatMod(4, 1);
        ...
        {
            int dust = Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, DustID.DesertTorch, Projectile.velocity.X / 2, Projectile.velocity.Y / 2, 100, default(Color), 0f);
			...
			if(pokemonProj != null && pokemonProj.active && pokemonProj.velocity.Length() > 0.2f)
			{
				...
				for(int i = 0; i < 2; i++)
				{
					dust = Dust.NewDust(pokemonProj.Center - 0.5f*new Vector2(pokemonProj.width,pokemonProj.height), pokemonProj.width, pokemonProj.height, DustID.Torch, Projectile.velocity.X / 2, Projectile.velocity.Y / 2, 100, default(Color), 0f);
					Main.dust[dust].noGravity = true;
					Main.dust[dust].scale = Main.rand.NextFloat(1f,2f);
				}
			}
