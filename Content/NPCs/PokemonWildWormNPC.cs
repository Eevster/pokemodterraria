...
namespace Pokemod.Content.NPCs
{
	...
	public abstract class WormPokemonNPC : PokemonWildNPC
	{
		public override string Texture => "Pokemod/Assets/Textures/Pokesprites/Pets/Extras/"+pokemonName+(shiny?"PetProjectileShiny":"PetProjectile")+"_Chain";
		//public override string Texture => "Pokemod/Assets/Textures/Pokesprites/Pets/Extras/"+pokemonName+"PetProjectile"+"_Chain";
		/*  ai[] usage:
		...
		public NPC FollowingNPC = null;
		public NPC FollowerNPC = null;
        ...
        {
			if(SegmentType != WormSegmentType.Head && NPC.realLife != -1 && HeadSegment is not null && HeadSegment.ModNPC is PokemonWildNPC nPC) hostilePokemon = nPC.hostilePokemon;
            base.AI();
        }

        public override void OnSpawn(IEntitySource source)
        {
            if(SegmentType == WormSegmentType.Head) base.OnSpawn(source);
			else
			{
		...
		public override void Movement() {
			...
			if (SegmentType == WormSegmentType.Head) {
				...
				if(AI_Timer <= 0 && Main.netMode != NetmodeID.MultiplayerClient){
					...
					if(moveDirection == 0 && flyDirection == 0) moveDirection = 1; 
        ...
        {
            if(SegmentType == WormSegmentType.Head) base.SendExtraAI(writer);
        }

        public override void ReceiveExtraAI(BinaryReader reader)
        {
            if(SegmentType == WormSegmentType.Head) base.ReceiveExtraAI(reader);
        }

		public override void OnHitByItem(Player player, Item item, NPC.HitInfo hit, int damageDone)
        {
			if(SegmentType == WormSegmentType.Head) hostilePokemon = true;
			else if(HeadSegment is not null && HeadSegment.ModNPC is PokemonWildNPC nPC)
			{
				nPC.hostilePokemon = true;
			}
		...
		public static Vector2 FindSpot(Vector2 initialPos, float minFleeDistance, float fleeDistance, int searchWidth, int fleeXDirection, int fleeYDirection, out bool spotFound)
        {
            int searchRange = (int)(fleeDistance / 16f);
			int minSearchRange = (int)(minFleeDistance / 16f);
            spotFound = false;

            int tileX = (int)(initialPos.X / 16f);
            int tileY = (int)(initialPos.Y / 16f);

			Vector2 spotPosition = 16 * new Vector2(tileX, tileY);

			Vector2 searchVector = new Vector2(fleeXDirection,fleeYDirection);

			Vector2 searchPosition;

            for (int i = minSearchRange; i < searchRange; i++)
            {
                searchPosition = new Vector2(tileX, tileY) + i*searchVector;

				if(searchPosition.X < 0 || searchPosition.X >= Main.maxTilesX || searchPosition.Y < 0 || searchPosition.Y >= Main.maxTilesY-1) break;

				for (int j = 0; j < searchWidth; j++)
				{
					searchPosition = new Vector2(tileX, tileY) + i*searchVector + (j-searchWidth/2)*new Vector2(searchVector.Y,searchVector.X);

					if(!(searchPosition.X < 0 || searchPosition.X >= Main.maxTilesX || searchPosition.Y < 0 || searchPosition.Y >= Main.maxTilesY-1)){
						Tile tileAir = Main.tile[(int)searchPosition.X, (int)searchPosition.Y];
						if (!tileAir.HasTile || !Main.tileSolid[tileAir.TileType])
						{
							Tile tileGround = Main.tile[(int)searchPosition.X, (int)searchPosition.Y+1];
							if(tileGround.HasTile && Main.tileSolid[tileGround.TileType])
							{
								spotPosition = new Vector2((int)searchPosition.X * 16, (int)searchPosition.Y * 16);
								spotFound = true;
	...
	/// </summary>
	public abstract class WormPokemonHead : WormPokemonNPC
	{
		...
		private void HeadAI_SpawnSegments() {
			if (Main.netMode != NetmodeID.MultiplayerClient) {
				...
				bool hasFollower = FollowerNPC != null;
				if (!hasFollower) {
		...
		private void HeadAI_Movement(bool collision) {
			...
			float speed = collision?5*moveSpeed:moveSpeed;
			...
			Vector2 pointTarget = FindSpot(NPC.Center, 12*16, 40*16, 5, moveDirection, flyDirection, out bool spotFound);
			if(spotFound){
				prevTarget = pointTarget;
			}
			else
			{
				if(Vector2.Distance(prevTarget, NPC.Center) < 40 * 16) pointTarget = prevTarget;
				else pointTarget = NPC.Center;
			}

			//Main.NewText((pointTarget-NPC.Center) + " DirectionVector: ("+moveDirection+", "+flyDirection+")" + " Colision: "+collision);

			(targetXPos, targetYPos) = (pointTarget.X, pointTarget.Y);
	...
	public abstract class WormPokemonBody : WormPokemonNPC
	{
		...
		internal static void CommonAI_BodyTail(WormPokemonNPC worm) {
			...
			NPC following = worm.FollowingNPC;
			if (Main.netMode != NetmodeID.MultiplayerClient) {
				...
				if (following is null || !following.active || following.townNPC || following.lifeMax <= 5) {
					worm.NPC.life = 0;
